...
| `apps/web` | Real engine + persistence: IndexedDB op store (`src/persist/`, HLC continuity across restarts, cached fold), capture panel (create track, log instant, start/stop ongoing), demo/my-data source toggle, gcal connect UI (`src/gcal-ui/`: calendar list, dry-run report, per-cluster import, incremental pull with resync-replace semantics), four views on either data source (radial with hour numerals + per-ring date labels + toggleable S-connectors on boundary crossings, day-circles small multiples, columns, year grid) |
